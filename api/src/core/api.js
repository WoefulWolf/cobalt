...
export const runAPI = async (express, app, __dirname, isPrimary = true) => {
      ...
      if (env.shortenUrls) {
        if ("url" in result.body && result.body.status == "redirect") {
          const metadata = {
            service: parsed.host,
            patternMatch: parsed.patternMatch,
            originalUrl: normalizedRequest.url.toString(),
          };
          const id = URLShortener.shortenUrl(result.body.url, metadata);
  ...
  if (env.shortenUrls) {
    app.get("/:id", async (req, res) => {
      const [cleanId, extraParams] = req.params.id
        .split(/[&?](.*)/s)
        .filter(Boolean);
      let longUrl = URLShortener.resolveUrl(cleanId);

      if (!longUrl) {
        return res.status(404).send("Not found");
      }

      // Check if it's an expired Instagram CDN URL
      if (URLShortener.isInstagramUrlExpired(longUrl)) {
        const metadata = URLShortener.getUrlMetadata(cleanId);

        // If we have metadata, try to regenerate the URL
        if (
          metadata &&
          metadata.service === "instagram" &&
          metadata.patternMatch
        ) {
          try {
            const normalizedRequest = normalizeRequest(req);
            const result = await match({
              host: "instagram",
              patternMatch: metadata.patternMatch,
              params: {
                ...normalizedRequest,
                url: new URL(metadata.originalUrl),
              },
              authType: req.authType ?? "none",
            });

            if (result.body.status === "redirect" && result.body.url) {
              // Update the shortened URL with the new CDN URL
              URLShortener.updateUrl(cleanId, result.body.url);
              longUrl = result.body.url;
            }
            console.info("Regenerated expired Instagram URL for:", cleanId);
          } catch (err) {
            // If regeneration fails, just redirect to the old URL
            // (it might still work or show an Instagram error)
            console.error("Failed to regenerate expired Instagram URL:", err);
          }
        }
      }
